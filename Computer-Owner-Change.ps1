...
$logFileName = "$env:computername-$logtimestamp-SetDomainAdminsASOwner-ComputerObjects.log"
...
$toplevelOU = Get-ADOrganizationalUnit -Filter * -Server $DomainServer -SearchBase $($Domaindata.DistinguishedName) -SearchScope subtree |
 ...
 [array]$AllObjects  = $null                     
    Foreach ($OU in $toplevelOU){
        Try {
                [array]$ComputerObjects = $null

                Write-host "Working on $OU"
                $ComputerObjects = Get-ADComputer -Filter * -SearchBase $OU -SearchScope OneLevel -Server $DomainServer -Properties Name, ObjectClass,nTSecurityDescriptor | 
                    Select-Object Name,ObjectClass,DistinguishedName ,nTSecurityDescriptor, @{Name="Owner"; Expression={$($_.nTSecurityDescriptor).Owner}} 
                    Write-Host "Computer count is $(($ComputerObjects| Measure-Object).count)"
                If($(($ComputerObjects| Measure-Object).count) -gt '0'){
                    $ComputerObjects | Export-csv $PSScriptRoot\ComputerObjects.csv -NoTypeInformation -Append
                }
        ...
        $AllObjects = Import-csv  $PSScriptRoot\ComputerObjects.csv
