...
AD object types: Organizational Unit
-------------------------------------------
...
$logFileName = "$env:computername-$logtimestamp-SetDomainAdminsASOwner-Computer.log"
...
$toplevelOU = Get-ADOrganizationalUnit -Filter * -Server $DomainServer -SearchBase $($Domaindata.DistinguishedName) -SearchScope subtree |
 ...
 [array]$AllObjects  = $null                     
    Foreach ($OU in $toplevelOU){
        Try {
                [array]$OUObjects = $null

                Write-host "Working on $OU"
                $OUObjects = Get-ADOrganizationalUnit -Identity $OU -Server $DomainServer -Properties Name, ObjectClass,nTSecurityDescriptor | 
                    Select-Object Name,ObjectClass,DistinguishedName ,nTSecurityDescriptor, @{Name="Owner"; Expression={$($_.nTSecurityDescriptor).Owner}} 
                    Write-Host "OU count is $(($OUObjects| Measure-Object).count)"
                If($(($OUObjects| Measure-Object).count) -gt '0'){
                    $OUObjects | Export-csv $PSScriptRoot\OUObjects.csv -NoTypeInformation -Append
                }
            }catch {
        ...
        $AllObjects = Import-csv  $PSScriptRoot\OUObjects.csv
