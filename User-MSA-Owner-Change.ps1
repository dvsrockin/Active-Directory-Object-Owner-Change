...
AD object types:User and Managed Service Account
-------------------------------------------
...
$logFileName = "$env:computername-$logtimestamp-SetDomainAdminsASOwner-UsernMSAObjects.log"
...
$toplevelOU = Get-ADOrganizationalUnit -Filter * -Server $DomainServer -SearchBase $($Domaindata.DistinguishedName) -SearchScope subtree |
 ...
 [array]$AllObjects  = $null                     
    Foreach ($OU in $toplevelOU){
        Try {
                [array]$UserObjects = $null
                [array]$MSAObjects  = $null

                Write-host "Working on $OU"
                $UserObjects = Get-ADUser -Filter * -SearchBase $OU -SearchScope OneLevel -Server $DomainServer -Properties Name, ObjectClass,nTSecurityDescriptor | 
                    Select-Object Name,ObjectClass,DistinguishedName ,nTSecurityDescriptor, @{Name="Owner"; Expression={$($_.nTSecurityDescriptor).Owner}} 
                    Write-Host "User count is $(($UserObjects| Measure-Object).count)"
                If($(($UserObjects| Measure-Object).count) -gt '0'){
                    $UserObjects | Export-csv $PSScriptRoot\UsernMSA.csv -NoTypeInformation -Append
                }

                $MSAObjects =    Get-ADServiceAccount -Filter *  -SearchBase $OU -SearchScope OneLevel -Server $DomainServer -Properties Name, ObjectClass,nTSecurityDescriptor | 
                    Select-Object Name,ObjectClass,DistinguishedName ,nTSecurityDescriptor, @{Name="Owner"; Expression={$($_.nTSecurityDescriptor).Owner}} 
                    Write-Host "MSA count is $(($MSAObjects| Measure-Object).count)"
                If($(($MSAObjects| Measure-Object).count) -gt '0'){
                    $MSAObjects | Export-csv $PSScriptRoot\UsernMSA.csv -NoTypeInformation -Append
                }
        ...
        $AllObjects = Import-csv  $PSScriptRoot\UsernMSA.csv
